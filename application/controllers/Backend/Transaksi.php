...
class Transaksi extends CI_Controller
{
    ...
    public function show($id = "")
    {
        ...
        $where1 = '(trans_tipe="1" or trans_tipe = "2")';
        $this->db->select('*');
        ...
        foreach ($query as $key => $q) {
            if (!empty($id)) :
                ...
                $getService = $this->modelFunction->getAllById('data_trans_service', 'id_trans', $id);
    ...
    public function insert()
    {
        ...
        $idCheckServ  = $this->input->post('chservice', TRUE);
        $tipeTrans = 0;
        ...
        // $setId = strtoupper('INV' . date('dmy') . random_string('alnum', 2)  . date('his') . random_string('alnum', 2));
        // $setupId  = $this->input->post('trans_id', TRUE);
        $setupId  = strtoupper('TR-' . random_string('alpha', 2) . date('Ymd')   . date('his'));

        foreach ($idCheckServ as $key1 => $val1) :
            $newIdServ     = $_POST['chservice'][$key1];
            $getServData = $this->modelFunction->getAllById('data_service', 'service_id', $newIdServ);

            foreach ($getServData as $sr) {
                $serviceNm    = $sr['service_nm'];
                $serviceHrg   = $sr['service_hrg'];
                $subTotalServ += $serviceHrg;
            }

            $dataBatch2[] = array(
                'id_trans'     => $setupId,
                'service_nm'   => $serviceNm,
                'service_hrg'  => $serviceHrg,
            );
        endforeach;

        $this->modelFunction->insert_batch('data_trans_service', $dataBatch2);

        if (!empty($idSparepart)) :
            ...
            endforeach;
            $this->modelFunction->insert_batch('data_trans_spart', $dataBatch1);
            $grandTotal = $subTotalServ + $subTotalSpr;
            $tipeTrans  = 1;

        else :
            $grandTotal = $subTotalServ;
            $tipeTrans  = 2;
        endif;

        $result = [
            'trans_id'      => $setupId,
            'trans_tipe'    => $tipeTrans,
            'cust_nm'       => $this->input->post('cust_nm'),
            'cust_tlp'      => $this->input->post('cust_tlp'),
            'cust_motor'    => $this->input->post('motor'),
    ...
    public function updateStatus($id)
    {
        ...
        if ($store) {
            ...
            if (!empty($getSpartSelling)) {
                // GETING DATA TRANS FOR TRANSFER TO PENJUALAN
