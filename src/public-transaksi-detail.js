...
var tableService = pageName.find(".table-service");
...
$.getJSON(basedApi + "/show/" + thisId, function (resp) {
	...
	/** Data Service */
	dataService.push(resp.data.data_service);
	$.each(dataService, function (index) {
		var thisData = dataService[index];
		var optionService = "";
		for (let i = 0; i < thisData.length; i++) {
			optionService += `<tr>`;
			optionService += `<td class="wd-20p">${thisData[i].service_nm}</td>`;
			optionService += `<td class="wd-5p tx-center"> </td>`;
			optionService +=
				`<td class="wd-10p tx-right">` +
				formatRupiah(thisData[i].service_hrg, "Rp. ") +
				`</td>`;
			optionService +=
				`<td class="wd-10p tx-right">` +
				formatRupiah(thisData[i].service_hrg, "Rp. ") +
		...
		tableService.append(optionService);
	});
	...
	// for (let i = 0; i < 5; i++) {
	// 	optionService += `<tr>`;
	// 	optionService += `<td class="wd-20p">a</td>`;
	// 	optionService += `<td class="wd-5p tx-center">b</td>`;
	// 	optionService += `<td class="wd-10p tx-right">c</td>`;
	// 	optionService += `<td class="wd-10p tx-right">d</td>`;
	// 	optionService += `</tr>`;
	// }
	// tableService.append(optionService);
